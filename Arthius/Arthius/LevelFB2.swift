...
public final class LevelData {
	...
	public var levelConditions : LevelConditions? = nil
	...
	public init(){}
	public init(levelMetadata: LevelMetadata?, levelConditions: LevelConditions?, texts: [LevelText], propFrame: Rect?, endPoints: [EndData], lineData: [LineData], gravityWells: [GravityWellData], colorBoxData: [ColorBoxData], rockData: [RockData], antiGravityZones: [AntiGravityData]){
		self.levelMetadata = levelMetadata
		self.levelConditions = levelConditions
		self.texts = texts
...
public extension LevelData {
	fileprivate static func create(_ reader : FlatBuffersReader, objectOffset : Offset?) -> LevelData? {
		...
		_result.levelConditions = LevelConditions.create(reader, objectOffset: reader.offset(objectOffset: objectOffset, propertyIndex: 1))
		let offset_texts : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 2)
		...
		_result.propFrame = reader.get(objectOffset: objectOffset, propertyIndex: 3)
		let offset_endPoints : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 4)
		...
		let offset_lineData : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 5)
		...
		let offset_gravityWells : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 6)
		...
		let offset_colorBoxData : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 7)
		...
		let offset_rockData : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 8)
		...
		let offset_antiGravityZones : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 9)
...
public struct LevelData_Direct<T : FlatBuffersReader> : Hashable, FlatBuffersDirectAccess {
	...
	public var levelConditions : LevelConditions_Direct<T>? { get { 
		if let offset = reader.offset(objectOffset: myOffset, propertyIndex: 1) {
			return LevelConditions_Direct(reader: reader, myOffset: offset)
		}
		return nil
	} }
	...
	public var antiGravityZones : FlatBuffersTableVector<AntiGravityData_Direct<T>, T> {
		let offsetList = reader.offset(objectOffset: myOffset, propertyIndex: 9)
		return FlatBuffersTableVector(reader: self.reader, myOffset: offsetList)
	}
	public var hashValue: Int { return Int(myOffset) }
}
...
public extension LevelData {
	fileprivate func addToByteArray(_ builder : FlatBuffersBuilder) throws -> Offset {
		...
		let offset1 = try levelConditions?.addToByteArray(builder) ?? 0
		let offset0 = try levelMetadata?.addToByteArray(builder) ?? 0
		try builder.startObject(withPropertyCount: 10)
		if antiGravityZones.count > 0 {
			try builder.insert(offset: offset9, toStartedObjectAt: 9)
		}
		if rockData.count > 0 {
...
public final class LevelConditions {
	public var maxDistance : Float32 = 0
	public var maxTime : Float32 = 0
	public init(){}
	public init(maxDistance: Float32, maxTime: Float32){
		self.maxDistance = maxDistance
		self.maxTime = maxTime
	}
}
public extension LevelConditions {
	fileprivate static func create(_ reader : FlatBuffersReader, objectOffset : Offset?) -> LevelConditions? {
		guard let objectOffset = objectOffset else {
			return nil
		}
		if  let cache = reader.cache,
			let o = cache.objectPool[objectOffset] {
			return o as? LevelConditions
		}
		let _result = LevelConditions()
		if let cache = reader.cache {
			cache.objectPool[objectOffset] = _result
		}
		_result.maxDistance = reader.get(objectOffset: objectOffset, propertyIndex: 0, defaultValue: 0)
		_result.maxTime = reader.get(objectOffset: objectOffset, propertyIndex: 1, defaultValue: 0)
		return _result
	}
}
public struct LevelConditions_Direct<T : FlatBuffersReader> : Hashable, FlatBuffersDirectAccess {
	fileprivate let reader : T
	fileprivate let myOffset : Offset
	public init?<R : FlatBuffersReader>(reader: R, myOffset: Offset?) {
		guard let myOffset = myOffset, let reader = reader as? T else {
...
public extension LevelConditions {
	fileprivate func addToByteArray(_ builder : FlatBuffersBuilder) throws -> Offset {
		...
		try builder.insert(value : maxTime, defaultValue : 0, toStartedObjectAt: 1)
		try builder.insert(value : maxDistance, defaultValue : 0, toStartedObjectAt: 0)
		let myOffset =  try builder.endObject()
		if builder.options.uniqueTables {
			builder.cache[ObjectIdentifier(self)] = myOffset
		}
...
public final class LevelMetadata {
	...
	public var levelThumbnail : Rect? = nil
	public init(){}
	public init(levelUUID: String?, levelNumber: Int32, levelName: String?, levelVersion: String?, levelAuthor: String?, levelThumbnail: Rect?){
		self.levelUUID = levelUUID
		self.levelNumber = levelNumber
		self.levelName = levelName
		self.levelVersion = levelVersion
		self.levelAuthor = levelAuthor
		self.levelThumbnail = levelThumbnail
	}
}
public extension LevelMetadata {
	fileprivate static func create(_ reader : FlatBuffersReader, objectOffset : Offset?) -> LevelMetadata? {
		...
		_result.levelThumbnail = reader.get(objectOffset: objectOffset, propertyIndex: 5)
		return _result
	}
}
public struct LevelMetadata_Direct<T : FlatBuffersReader> : Hashable, FlatBuffersDirectAccess {
	...
	public var levelThumbnail : Rect? { 
...
public extension LevelMetadata {
	fileprivate func addToByteArray(_ builder : FlatBuffersBuilder) throws -> Offset {
		...
		if let levelThumbnail = levelThumbnail {
			builder.insert(value: levelThumbnail)
