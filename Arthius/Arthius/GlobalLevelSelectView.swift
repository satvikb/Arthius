...
//    func globalLevelSelect_getLevels(query : LevelQuery) -> [GLSLevelData]
    func globalLevelSelect_getLevels(query: LevelQuery, completion: @escaping ([LevelData], [String:Int]) -> Void)
...
class GlobalLevelSelectView : UIView, GLSSelectorDelegate {
    ...
    override init(frame: CGRect) {
        ...
        levelSelector = GLSLevelSelector(frame: propToRect(prop: CGRect(x: 0, y: 0.25, width: 1, height: 0.75)), xTiles: 3, yTiles: 3, levels: [], downloads: [:])
    ...
    func loadLevels(query : LevelQuery){
        ...
        globalLevelSelectDelegate?.globalLevelSelect_getLevels(query: query, completion: {(levelData : [LevelData], downloads: [String: Int]) in
            
            print("Got levels: \(String(describing: levelData.count))")

            //TODO create/update level selector
            self.levelSelector.updateLevels(self.levelSelector.xTiles, self.levelSelector.yTiles, newLevels: levelData, downloads: downloads)
        })
